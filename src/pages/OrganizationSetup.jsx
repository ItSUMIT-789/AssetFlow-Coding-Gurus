...
export default function OrganizationSetup() {
  ...
  const loadDepartments = () => {
    ...
    return getAll(KEYS.departments).map((d) => ({
      ...d,
      code: d.code || d.id.replace("DEP-", ""),
      status: d.status === "ACTIVE" ? "Active" : "Inactive",
      employees: users.filter((u) => u.department === d.name).length,
    }));
  };
  const [departments, setDepartments] = useState(() =>
      loadDepartments().length ? loadDepartments() : seedDepartments,
    ),
  ...
  const save = (form) => {
    ...
    if (modal.mode === "add") storeCreate(KEYS.departments, payload);
    else storeUpdate(KEYS.departments, modal.department.id, payload);
          ...
          >
            ...
            >
              ...
              <div className="mt-6 flex justify-end gap-3">
                ...
                <button
                  onClick={() => {
                    storeRemove(KEYS.departments, deleteTarget.id);
                    setDepartments(loadDepartments());
...
function SecondaryPanel({ type, categories, setCategories, users, setUsers }) {
  const [name, setName] = useState(""),
    [code, setCode] = useState("");
  if (type === "Categories") {
    const add = () => {
      if (!name.trim() || !code.trim()) return;
      storeCreate(KEYS.categories, {
        name: name.trim(),
        code: code.trim().toUpperCase(),
        bookable: false,
      });
      setCategories(getAll(KEYS.categories));
      setName("");
      setCode("");
    };
    return (
      <div className="p-5">
        <div className="grid gap-3 rounded-2xl bg-slate-50 p-4 dark:bg-slate-950 sm:grid-cols-[1fr_160px_auto]">
          <input
            value={name}
            ...
            <tbody>
              {categories.map((c) => (
                <tr key={c.id} className="border-t dark:border-slate-800">
                  ...
                  <td>{c.code}</td>
                  <td>{c.bookable ? "Yes" : "No"}</td>
                  <td>
                    <button
                      onClick={() => {
                        const next = window.prompt("Category name", c.name);
                        if (next) {
                          storeUpdate(KEYS.categories, c.id, { name: next });
                          setCategories(getAll(KEYS.categories));
                        }
                      }}
              ...
              <tr key={u.id} className="border-t dark:border-slate-800">
                ...
                <td>
                  <select
                    value={u.role}
                    onChange={(e) => {
                      storeUpdate(KEYS.users, u.id, { role: e.target.value });
                      setUsers(getAll(KEYS.users));
                    }}
                    className="rounded-lg border px-2 py-1.5 text-xs dark:bg-slate-900"
                  >
