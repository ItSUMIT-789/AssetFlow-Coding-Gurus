...
export default function AssetUtilizationChart({ data, title = 'Asset utilization', copy = 'Allocated, available, maintenance, and retired assets from the shared dataset.' }) {
  const analytics = useAssetFlow()
  const chartData = data ?? analytics.statusDistribution

  return (
    <Panel>
      <div className="mb-5 flex items-end justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-[.24em] text-sky-600 dark:text-sky-300">Utilization</p>
      ...
      <div className="h-72">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={chartData} dataKey="value" nameKey="name" innerRadius={70} outerRadius={108} paddingAngle={4}>
