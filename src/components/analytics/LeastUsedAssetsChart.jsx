...
export default function LeastUsedAssetsChart({ data, title = 'Least used assets', copy = 'Lower-frequency items can indicate underutilized inventory or reserve equipment.' }) {
  const analytics = useAssetFlow()
  const chartData = data ?? analytics.leastUsedAssets
        ...
        {chartData.map((item, index) => (
          <div key={item.id} className="rounded-2xl border border-slate-200/70 bg-white/75 p-4 dark:border-white/10 dark:bg-white/5">
            <div className="flex items-start justify-between gap-4">
              ...
              <strong className="rounded-full bg-rose-500/10 px-3 py-1 text-sm font-black text-rose-700 dark:text-rose-200">{item.usageCount}</strong>
            </div>
          </div>
        ))}
      </div>
